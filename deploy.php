 ...
 */
task('preparando_servidor', function () {
 ...
 * Release
 */
task('preparando_release', function () {
 ...
 * Update project code
 */
task('atualizando_codigo', function () {
 ...
 */
task('copia_diretorios', function () {
    $dirs = get('copy_dirs');
    foreach ($dirs as $dir) {
 ...
 * Make writable dirs.
 */
task('setando_permissoes_diretorio', function () {
 ...
 */
task('instalando_bibliotecas', function () {
 ...
 */
task('removendo_releases_antigos', function () {
    $releases = env('releases_list');
...
task('success', function () {
    writeln("<info>Implantação realizada com sucesso!</info>");
})
    ->once()
    ->setPrivate();

task('renomeando_arquivos', function () {
	run("cd {{deploy_path}}/current && mv .htaccess.dist .htaccess");
	run("cd {{deploy_path}}/current/application/config && mv config.php.dist config.php");
...
task('executa_implantacao', [
    ...
    'preparando_release',
    'atualizando_codigo',
    'setando_permissoes_diretorio',
    'instalando_bibliotecas',
    'criando_symlink',
